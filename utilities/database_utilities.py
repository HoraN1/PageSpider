...
def create_database(database_path: str):
    ...
    with conn:
        ...
        ddl = "create table words (word text not null constraint words_pk primary key, usage_count int default 1 not " \
              "null) "
        cur.execute(ddl)
...
def save_words_to_db(database_path: str, words_list: list):
    conn = lite.connect(database_path)
    with conn:
        cur = conn.cursor()
